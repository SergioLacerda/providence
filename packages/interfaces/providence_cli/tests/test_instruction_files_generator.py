...
def _providence_integration_template(rel: str) -> Path:
    import providence_integration

    return Path(providence_integration.__file__).parent / "templates" / rel


def test_static_claude_instructions_no_spec_config() -> None:
    f = _providence_integration_template(".claude/claude-instructions.md")
    if not f.exists():
        return
...
def test_static_gemini_instructions_no_spec_config() -> None:
    f = _providence_integration_template(".gemini/gemini-instructions.md")
    if not f.exists():
        return
    content = f.read_text(encoding="utf-8")
    assert ".spec.config" not in content, (
...
def test_no_legacy_antigravity_template_shipped() -> None:
    ...
    f = _providence_integration_template(".antigravity/antigravity-instructions.md")
    assert not f.exists(), (
...
# ---------------------------------------------------------------------------
# Antigravity regenerated by providence governance generate
# ---------------------------------------------------------------------------


def test_antigravity_regenerated_by_governance_generate(tmp_path: Path) -> None:
    """providence governance generate must produce antigravity instructions."""
    generate_agent_instruction_files(tmp_path, _sample_config())
    antigravity_file = (
...
# ---------------------------------------------------------------------------
# VSCode/Cursor: no inline mandate descriptions after providence governance generate
# ---------------------------------------------------------------------------
